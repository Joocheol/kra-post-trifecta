...
부분식별 경계가 실제로 동등한 지위로 구현됐는지를 판단한다.
...
직전 최종 검토의 F1--F5에 따라 이번 후속에서는 다음을 추가했다.
...
- 표 2의 삼쌍승 원천행과 untracked CSV의 manifest 해시 검증경로 설명

이 다섯 수정이 직전 지적을 실제로 닫는지 확인하고, 이미 해결된 연구범위 자체를
다시 논점으로 만들지 않는다.

그 후속 검토에서 발견된 두 비문은 완결된 문장으로 교정했고, Panel A/B의 패널
집계는 경주균등가중 중앙값으로 확정했다. 기준모형 우위는 Panel A의 TV 개선폭
중앙값과 Panel B의 보수적 개선폭 하한 중앙값 각각에 대해 경주 부트스트랩 95\%
신뢰구간 하한이 0보다 큰지를 사용한다. 절대 TV 기준은 0.05이며 0.025와 0.10을
민감도로 보고한다. 이 PR은 표본·설계 확정 단계이고 LP 결과 코드는 후속 PR에서
작성한다.
