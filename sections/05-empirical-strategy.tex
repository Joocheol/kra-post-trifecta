...
$\overline{\operatorname{TV}}^{\rm out}_{\rm main}
 <\underline{\operatorname{TV}}_{\rm benchmark}$일 때만 내린다. 구간이 겹치면 순위를
...
강한 주결론은 Panel A의 점추정과 Panel B의 보수적 TV/MAE 경계가 같은 방향을
지지할 때만 내린다. Panel A에서는 기준모형보다 우월하지만 Panel B에서 주모형
TV 상한이 기준모형 TV 하한보다 작지 않으면 `clean 표본에서만 확인`으로 제한한다.
