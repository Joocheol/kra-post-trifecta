...
\resizebox{\textwidth}{!}{\input{tables/main_sample_selection}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/main_heterogeneity_comparison}}
\end{table}
...
\resizebox{\textwidth}{!}{\input{tables/main_panel_a}}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Panel B: 전체표본의 TV 부분식별 경계}
\label{tab:main-panel-b}
\resizebox{\textwidth}{!}{\input{tables/main_panel_b}}
\end{table}
...
전체표본의 보수적 상한까지 고려하면 이 강한 기준을 인증하지 못한다. 반면
민감도 기준 0.10에서는 두 패널이 모두 통과한다. 따라서 주결론은 ``정확한
등식''보다 ``기준모형을 크게 능가하는 높은 교차풀 정합성, 그러나 무시할 수
없는 잔여 가격차''로 제한한다.
...
\begin{table}[htbp]
\centering
\caption{기준모형 대비 TV 개선폭}
\label{tab:main-benchmarks}
\resizebox{\textwidth}{!}{\input{tables/main_benchmark_comparison}}
\end{table}
...
\resizebox{0.86\textwidth}{!}{\input{tables/main_order_information}}
\end{table}
...
\resizebox{0.96\textwidth}{!}{\input{tables/main_order_information_bounds}}
\end{table}

\begin{table}[htbp]
\centering
\caption{P3 공유 삼쌍승 가격집합을 강제한 대표경주 sharpness 진단}
...
오는지 확인하기 위해, 출전두수 5--16두의 대표경주 한 개씩에서 동일한 삼쌍승
가격변수가 두 하위 승식 주변분포를 동시에 생성하도록 강제한 joint MILP 결과를
보인다. 인증구간의 폭은 기존 보수구간의 약 14--35\%로 크게 줄어든다. 5--7두와
12--14두 대표경주에서는 양(+)으로, 9두 대표경주에서는 음(-)으로 부호가
식별되지만, 8·10·11·15·16두 대표경주에서는 여전히 0을 포함한다. 16두의
최소화 문제는 제한시간 내 최적성을 완전히 인증하지 못해 그 하한은 certified
outer bound로 보고한다. 따라서 공유가격 제약을 정확히 반영하면 P3 경계는
상당히 날카로워지지만, 대표 출전두수 전반에서 방향이 일관되지 않으므로 전체
시장에서 쌍승의 순서정보 효과가 항상 양수라는 결론으로 강화할 수는 없다.
